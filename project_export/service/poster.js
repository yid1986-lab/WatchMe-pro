...
async function sendLivePost(client, guildId, data) {
  ...
  const linkedMember = getLinkedMember(guildId, data.platformKey, data.sourceUrl || data.url, data.externalId);
  ...
  if (Number(cfg.show_game ?? 1) === 1 && data.game) {
    embed.addFields({ name: data.platformKey === "youtube" ? "Category" : "Game", value: String(data.game).slice(0, 1024) });
  }
